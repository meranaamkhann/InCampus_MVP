...
public class AuthServiceImpl implements AuthService {
    ...
    public AuthResponse refresh(RefreshTokenRequest request) {
        ...
        if (!otpEncoder.matches(request.getRefreshToken(), user.getCurrentRefreshTokenHash())) {
            // Doesn't match the last-issued token: either stale (already rotated) or
            // stolen. Either way, revoke the session rather than silently failing,
            // so a leaked token can't be retried indefinitely.
            user.setCurrentRefreshTokenHash(null);
            user.setRefreshTokenExpiresAt(null);
            userRepository.save(user);
            throw ApiException.unauthorized("Refresh token invalid or already used - please log in again");
        }

        // Valid -> rotate: issue a brand new access + refresh token pair and
        // invalidate the one just used.
        return issueTokens(user);
    }
    ...
    private AuthResponse issueTokens(User user) {
        ...
        String rawRefreshToken = jwtService.generateRawRefreshToken(user.getId());
