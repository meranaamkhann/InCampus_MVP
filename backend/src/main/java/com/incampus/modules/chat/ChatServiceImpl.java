...
public class ChatServiceImpl implements ChatService {
    ...
    public MessageResponse sendMessage(UUID senderId, SendMessageRequest request) {
        ...
        // Live in-room delivery (the /topic/chat/{roomId} broadcast) happens in
        // ChatWebSocketController right after this call returns. This notify()
        // call is the separate "you have a new message" ping that reaches the
        // recipient even if they're not currently looking at this chat room.
        User recipient = room.getUserA().getId().equals(senderId) ? room.getUserB() : room.getUserA();
        notificationService.notify(recipient.getId(), senderId, NotificationType.MESSAGE, room.getId(),
