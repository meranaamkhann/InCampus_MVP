    ...
    public CommentResponse addComment(UUID postId, UUID authorId, CreateCommentRequest request) {
        ...
        if (!post.getAuthor().getId().equals(authorId)) {
            notificationService.notify(
                    post.getAuthor().getId(), authorId, NotificationType.COMMENT, post.getId(),
                    author.getName() + " commented on your post");
        }
        if (parent != null && !parent.getAuthor().getId().equals(authorId) && !parent.getAuthor().getId().equals(post.getAuthor().getId())) {
            notificationService.notify(
                    parent.getAuthor().getId(), authorId, NotificationType.COMMENT, post.getId(),
