...
public class ProjectServiceImpl implements ProjectService {
    ...
    public void requestToJoin(UUID projectId, UUID requesterId, JoinProjectRequest request) {
        ...
        notificationService.notify(
                project.getAuthor().getId(), requesterId, NotificationType.PROJECT_JOIN_REQUEST, project.getId(),
                requester.getName() + " requested to join \"" + project.getTitle() + "\"");
    }
    ...
    public void respondToRequest(UUID joinRequestId, UUID currentUserId, ProjectJoinStatus decision) {
        ...
        String verb = decision == ProjectJoinStatus.ACCEPTED ? "accepted" : "declined";
        notificationService.notify(
                joinRequest.getRequester().getId(), currentUserId, NotificationType.PROJECT_JOIN_REQUEST, joinRequest.getProject().getId(),
                "Your request to join \"" + joinRequest.getProject().getTitle() + "\" was " + verb);
    }
